...
class BaseEncoder(nn.Module):
    ...
    def forward(self, input):
        ...
        output = output.transpose(1,0)
            # (bs, seq_len, hid_dim(*2 if bidirectional))
...
class SenderDecoder(nn.Module):
    ...
    def get_per_step_logits(self, input, hidden, keys, values, length_mask):
        ...
        # Attention
        queries = output.transpose(1,0)
            # bs, 1, hid_dim
        weighted_values, weights = self.attn(queries, keys, values, length_mask)
    ...
    def init_batch(self, encoder_outputs, sem_embs):
        ...
        input = torch.zeros(1, bs, dtype=torch.long, device=self.device) + self.output_size + 2
            # [1, bs]
        hidden = self.init_hidden(bs)
            # [1, bs, hid_dim]

        fake_EOS = self.embedding(torch.zeros(bs, 1, dtype=torch.long, device=self.device)
                                  + self.output_size)
        fake_EOS_sem = self.embedding(torch.zeros(bs, 1, dtype=torch.long, device=self.device)
                                      + self.output_size + 1)
        keys = torch.cat([encoder_outputs, fake_EOS], dim=1)
            # bs, max_len, hid_dim
        values = torch.cat([sem_embs, fake_EOS_sem], dim=1)
            # bs, max_len, hid_dim
        length_mask = torch.zeros(bs, 1, keys.shape[1], dtype=torch.bool, device=self.device)

        return bs, input, hidden, keys, values, length_mask

    def forward(self, encoder_outputs, sem_embs, deterministic=False):
        bs, input, hidden, keys, values, length_mask = self.init_batch(encoder_outputs, sem_embs)
        sequence, per_step_logits, entropy, attn_weights = [], [], [], []
        for _ in range(self.max_length):
            logits, hidden, weights = self.get_per_step_logits(input, hidden, keys, values,
                                                               length_mask)
        ...
        per_step_logits = torch.cat([torch.stack(per_step_logits, 1), zeros], dim=-1)
            # bs, max_len + 1, out_dim
        entropy = torch.cat([torch.stack(entropy, 1), zeros], dim=-1)
            # bs, max_len + 1, out_dim
...
class ReceiverDecoder(SenderDecoder):
    ...
    def forward(self, encoder_outputs, sem_embs, lengths, force=None):
        ...
        length_mask = torch.arange(encoder_outputs.size(1),
                                   device=self.device)[None, :] >= lengths[:, None]
        length_mask = length_mask[:, None, :]
            # bs, 1, max_message_length
        ...
        per_step_logits = torch.stack(per_step_logits, 1)
            # bs, n_attrs, n_values
        attn_weights = torch.stack(attn_weights, 1)
...
class ModifSender(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.decoder = SenderDecoder(opts.vocab_size + 1, opts.hidden, opts.max_len,
                                     device=opts.device)
    ...
    def forward(self, x, aux_input=None, deterministic=False):
        ...
        bs = x.size(0)
        x = x.view(bs*self.n_attributes, self.n_values).nonzero()[:, 1].view(bs, self.n_attributes)

        enc_output, hidden, sem_embs = self.encoder(x)
        sequence, top_logits, entropy, attn_weights = self.decoder(enc_output, sem_embs,
...
class ModifReceiver(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.decoder = ReceiverDecoder(opts.n_values, opts.hidden, opts.n_attributes,
                                       device=opts.device)
    ...
    def forward(self, message, input=None, aux_input=None, lengths=None):
        ...
        per_step_logits, top_logits_, entropy_, attn_weights = self.decoder(enc_output, sem_embs,
                                                                            lengths)
