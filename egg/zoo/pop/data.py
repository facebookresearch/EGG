...
def select_ood_idxs(dataset):
    ...
    idx = [4491, 4624, 6114, 6774, 8786, 9333, 10518, 10548, 10731, 10862, 11361, 12012, 12069, 12121, 12491, 12640, 14127, 14129, 14131, 14150, 14172, 14257, 14280, 14373, 14559, 15124, 15214, 15412, 15669, 16334, 16514, 17094, 17299, 17301, 17350, 17377, 17384, 17401, 17594, 17603, 17611, 18151, 18808, 18847, 19100, 19213, 19481, 19548, 19578, 19644, 19705, 20127, 20356, 21650, 21662, 21688, 21741, 21765]
    # build the appropriate subset
    b = [torch.tensor(dataset.targets) == _id for _id in idx]
    _filter = b[0]
    for i in range(len(b)-1):
        _filter = _filter | b[i+1]
    return Subset(dataset, _filter.nonzero().reshape(-1))
