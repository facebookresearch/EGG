...
def select_ood_idxs(dataset):
    """
        from all imagenet classes select those which are inanimate
    """
    print("Selecting OOD classes - this could take a while", flush=True)
    ...
    idx = [i for i in range(len(dataset.imgs)) if (Path(dataset.imgs[:][i][0]).parent.stem in ood_ids)]
    #idx = [i for i in range(len(dataset)) if dataset.imgs[i][1] in ood_ids] 
    # build the appropriate subset
    print("Subset creation DONE", flush=True)
